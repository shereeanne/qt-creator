...
void IOutputPane::setupFilterUi(const Key &historyKey, const QString &actionSuffix)
{
    ...
    logQueryModeAction.addOnToggled(this, [this] {
        ...
        if (m_filterOutputLineEdit) {
            if (m_useLogQuery) {
                if (m_filterOutputLineEdit->text().isEmpty())
                    m_filterOutputLineEdit->setText(Tr::tr("Package:mine"));
            } else {
                m_filterOutputLineEdit->clear();
            }
        }
        updatePlaceholder();
        updateFilter();
...
void IOutputPane::filterOutputButtonClicked()
{
    ...
    const Utils::Id logQueryModeId = filterLogQueryModeActionId();
    for (const Utils::Id &id : m_additionalFilterActions) {
        // Only add log query mode action if supported
        if (id == logQueryModeId && !supportsLogQueryMode())
            continue;
